...
| **レイアウト帳票** | JSON ベースの独自形式で帳票を定義し直接印刷（台帳・調べ表・名列表 等） |
| **PDF/Word/Excel インポート** | 既存の帳票ファイルからレイアウトを取り込み |
| **レイアウトエディター** | 画面上でレイアウトの編集・プレビュー・印刷が可能 |
| **写真差込** | 児童写真をフォルダから自動マッチングし帳票に差し込み |
| **CSV 書き出し** | 名簿データを全校・学年・組・職員単位で CSV エクスポート |
...
| **鍵導出** | ユーザーパスワード + 環境キーから Argon2id で導出 |
...
| 項目 | 内容 |
|------|------|
| 自動テスト | **1,500 件以上**のテストケースを全リリースで実行 |
| コードカバレッジ | 47% 以上を CI で保証 |
